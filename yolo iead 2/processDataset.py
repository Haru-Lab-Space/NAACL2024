...
class ProcessDataset(nn.Module):
                    ...
                    if len(casual[j]) > 0:
                        ...
                        for span in casual[j]:
                            for k in range(len(casual_span_sample)):
                                ...
                                # print("span label "+str(k)+" : "+str(self.overlap(span, casual_text_sample[k], text[utter_ID-1])))
                                match.append(span_label[k])
                                ...
                                new_dataset.append({
                                    ...
                                    "paragraph": paragraph,
                                    "main_text": text[j],
                                    "text_pool": text[start_ids:end_ids],
                                    # "match": match,
    ...
    def convert2process_dataset(self, type="train"):
        ...
        for i, _ in enumerate(progress_bar):
            ...
            for j in range(len(data[i]['conversation'])):
                ...
                casual_text_augment = self.augmentModel.augment_sentence(text)
                casual_text = []
                casual_span = []
                for k in range(len(casual_text_augment)):
                    # print("text: "+str(text))
                    # print("casual_text_augment[k]: "+str(casual_text_augment[k]))
                    span = self.span(text, casual_text_augment[k])
                    if span != None:
                        ss, se, _ = span
                        # casual_text.append(casual_text[k])
                        # casual_span_pool.append((ss, se))
                        casual_text.append(casual_text_augment[k])
                ...
                casual_text, casual_span = self.remove_duplicate(casual_text, casual_span)

                # print("casual_text_augment: " + str(len(casual_text_augment)))
                # print("casual_text: " + str(len(casual_text)))
                # print("casual_span: " + str(len(casual_span)))
                # print("casual_text: " + str(casual_text))
        ...
        return new_data
    def remove_duplicate(self, pool, span_pool):
        
        index_list = [1] * len(pool)
        for i in range(len(index_list)-1):
            for j in range(i+1, len(index_list)):
                if span_pool[i][0] == span_pool[j][0]:
                    if span_pool[i][1] == span_pool[j][1]:
                        if span_pool[i][2] == span_pool[j][2]:
                            index_list[i] = 0
                            break
        ...
        for i in range(len(index_list)):
            if index_list[i] == 1:
                ...
                sentence_span_pool.append(span_pool[i])
        return sentence_pool, sentence_span_pool
    ...
    def overlap(self, text, sub_text, utter_text):
        ...
        if span_text == None:
            return 0
        start_span_text,end_span_text,_ = span_text
        start_sub_span_text, end_sub_span_text,_ = self.span(utter_text, sub_text)
        duplicate_span = min(end_span_text, end_sub_span_text) - max(start_span_text, start_sub_span_text)
        span = max(end_span_text, end_sub_span_text) - min(start_span_text, start_sub_span_text)
        # print("utter_text: "+str(utter_text))
        # print("sub_text: "+str(sub_text))
        # print("text: "+str(text))
        # print("duplicate_span: "+str(duplicate_span))
        # print("span: "+str(span))

        if duplicate_span < 0:
            return 0
        if duplicate_span/span > self.overlap_threshold:
            # print("duplicate_span/span: "+str(duplicate_span/span))
        ...
        for i in range(len(new_text)):
            ...
            for j in range(len(new_sub_text)):
                ...
                if new_sub_text[j] != new_text[i + j]:
                    # print("new_sub_text: "+str(new_sub_text))
