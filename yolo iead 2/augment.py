...
class AugmentData(nn.Module):
    ...
    def augment_sentence(self, text, test=False):
        ...
        sentence_pool = self.remove_list_punctuation_at_beginning(sentence_pool)
        if test:
            print("remove_list_punctuation_at_beginning:"+str(sentence_pool))
    ...
    def spacy(self, text):
        ...
        noun_chunks_pool=self.num_chunks(text)
    ...
    def num_chunks(self, text):
        ...
        for i in range(len(doc)):
            # Check if the token is a pronoun
            if doc[i].pos_ == 'NUM' or doc[i].text in delimiters:
                se = i+1
                span = ' '.join(words)
                sentence_pool.append(span)
                words = [doc[i].text]
    ...
    def remove_punctuation_at_beginning(self, input_str):
        # Remove leading punctuation using lstrip
        result = input_str.lstrip(string.punctuation).lstrip()
        return result
    
    def remove_list_punctuation_at_beginning(self, pool):
        sentence_pool = []

        for i in range(len(pool)):
            new_pool = self.remove_punctuation_at_beginning(pool[i])
            if len(new_pool) > 0:
                sentence_pool.append(new_pool)
        # print("pool: "+str(pool))
        # print("sentence_pool: "+str(sentence_pool))
        # print("pool: "+str(len(pool)))
        # print("sentence_pool: "+str(len(sentence_pool)))
        return sentence_pool